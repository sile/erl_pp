...
///
/// Build with [`Source::new`] after [`erl_tokenize::scan_tokens`], or with
/// [`Source::new`] alone when you already have a token stream from a
/// custom tokenizer or cache, or when your caller scans manually and
/// keeps going after lexical errors (formatters, linters, and language
/// servers that tolerate partial input often do this). Lexical errors from
/// [`erl_tokenize::scan_tokens`] are returned to the caller; they never
/// reach [`Preprocessor::step`](crate::Preprocessor::step).
#[derive(Debug, Clone)]
pub struct Source {
...
mod tests {
    ...
    fn new_accepts_external_tokens() {
        ...
        let tokens =
            erl_tokenize::scan_tokens(text).expect("test input must scan without lex errors");
        let scanned = Source::new("main.erl", text, tokens.clone());
        let by_new = Source::new("main.erl", text, tokens);
    ...
    fn append_and_get() {
        ...
        let id_a = store.append(source("a.erl", "a"));
        let id_b = store.append(source("b.erl", "bb"));
        assert_ne!(id_a, id_b);
    ...
    fn stable_addresses_after_further_append() {
        let store = SourceStore::new();
        let id = store.append(source("a.erl", "hello"));
        let first = store.get(id);
        let first_ptr = first.text().as_ptr();

        for i in 0..64 {
            store.append(source(&format!("f{i}.erl"), &format!("body {i}")));
        }
    ...
    fn fork_semantics_shared_store() {
        ...
        let id = store.append(source("main.erl", "-module(m)."));
        assert_eq!(fork.get(id).text(), "-module(m).");

        let fork_id = fork.append(source("inc.hrl", "-define(X, 1)."));
